...
export default function BillsScreen() {
	...
	return (
		<Background>
			<NewBillModal
				isBillModalActive={isModalActive}
				hideModal={() => setIsModalActive(false)}
			/>
			{bills.length > 0 || nearestBill != null ? (
				<View style={styles.containerBlur}>
					{isModalActive ? <View style={style.containerBlur}></View> : null}
					<View style={style.headerContainer}>
						<View style={style.BillContainer}>
							<Text style={style.BillTitle}>{t("bills:nearestBill")}</Text>
							<BillContainer bill={nearestBill} image={1} />
						</View>
						<View style={style.otherBillData}>
							<Text style={style.BillTitle}>{t("bills:allBills")}</Text>
							...
							<ScrollView style={style.scrollableBillsView}>
								{bills.map((bill) => {
									return (
										<BillContainer key={bill.name} bill={bill} image={2} />
									);
								})}
							</ScrollView>
						</View>
